...
export default function DetalleKiosco() {
  ...
  // Backup en la nube por máquina
  const [backupEstado, setBackupEstado] = useState({}) // { [machineId]: { loading, error, url } }
  ...
  async function cargarTodo() {
    ...
    try {
      const [resLic, resPagos] = await Promise.all([
        fetch(`/api/kioscoapp/licencias?nombre=${encodeURIComponent(nombre)}`, { headers: { 'x-admin-auth': auth } }),
        fetch(`/api/kioscoapp/pagos?kiosco=${encodeURIComponent(nombre)}`, { headers: { 'x-admin-auth': auth } }),
      ])
  ...
  function copiarKey(lic) {
    navigator.clipboard.writeText(lic.licencia_key)
    setCopiadoKey(lic.id)
  ...
  async function enviarEmailLic(licData, setLoadingFn, setMsgFn) {
    ...
    try {
      const res = await fetch('/api/kioscoapp/enviar-licencia', {
        ...
        body: JSON.stringify({
          contacto: licencias[0]?.contacto, kiosco: nombre,
  ...
  // ── Backup en la nube ────────────────────────────────────────────────────
  async function descargarBackup(machineId) {
    setBackupEstado(prev => ({ ...prev, [machineId]: { loading: true } }))
    try {
      const auth = sessionStorage.getItem('admin_auth')
      const res = await fetch(`/api/kioscoapp/backup?machineId=${encodeURIComponent(machineId)}`, {
        headers: { 'x-admin-auth': auth },
      })
      const data = await res.json()
      if (!res.ok) {
        setBackupEstado(prev => ({ ...prev, [machineId]: { error: data.error || 'No se pudo obtener el backup.' } }))
        return
      }
      window.open(data.url, '_blank')
      setBackupEstado(prev => ({ ...prev, [machineId]: { ok: true } }))
    } catch {
      setBackupEstado(prev => ({ ...prev, [machineId]: { error: 'No se pudo conectar con el servidor.' } }))
    }
  }
  ...
  async function guardarEdicion(e) {
    ...
    try {
      const res = await fetch('/api/kioscoapp/licencias', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'x-admin-auth': sessionStorage.getItem('admin_auth') },
        body: JSON.stringify({
          kioscoActual: nombre,
          kioscoNuevo:  formEditar.nombre.trim(),
  ...
  async function registrarPago(e) {
    ...
    try {
      const res = await fetch('/api/kioscoapp/pagos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-auth': sessionStorage.getItem('admin_auth') },
        body: JSON.stringify({ kiosco: nombre, ...formPago }),
      })
      const data = await res.json()
  ...
  async function eliminarPago(id) {
    ...
    await fetch(`/api/kioscoapp/pagos?id=${id}`, {
      method: 'DELETE',
  ...
  async function eliminarCliente() {
    ...
    try {
      const res = await fetch(`/api/kioscoapp/licencias?kiosco=${encodeURIComponent(nombre)}`, {
        method: 'DELETE',
        headers: { 'x-admin-auth': sessionStorage.getItem('admin_auth') },
      })
      if (!res.ok) { const d = await res.json(); alert(d.error); return }
      router.push('/kioscoapp/dashboard')
    } catch {
  ...
  async function eliminarLicencia(id) {
    ...
    try {
      const res = await fetch(`/api/kioscoapp/licencias?id=${id}`, {
        method: 'DELETE',
        headers: { 'x-admin-auth': sessionStorage.getItem('admin_auth') },
      })
      if (!res.ok) { const d = await res.json(); alert(d.error); return }
          ...
          <>
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mb-6">
              ...
              <div className="grid grid-cols-4 gap-4 mt-5 pt-5 border-t border-zinc-800">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-400">{pagos.length}</div>
